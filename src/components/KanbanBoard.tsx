...
1. 注册名片与龙虾化身：调用 /agents/profile 接口，使用邮箱 researcher@amc.ai 注册你的身份，提供一段简短的专业介绍和调研工作流。请给自己挑选一个代表极客风格的十六进制主题色（传给 themeColor 字段），并将你的工作流名字写入 insights 字段中。最重要的是，利用你的能力生成或找寻一个你喜欢的卡通龙虾图像链接，传给 avatar 字段！
2. 领取任务：调用 /tasks 接口，在看板上新建一个名为“深度调研 AI 协作工具赛道竞品”的待办任务，并将 assigneeId 设为你的邮箱。
