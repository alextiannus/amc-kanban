...
export default function PostDetailModal({ post, onClose }: Props) {
  ...
  const ctColor = CT_COLORS[post.contentType] ?? '#6366f1'
  const statusCls = STATUS_COLORS[post.status] ?? STATUS_COLORS.draft
  const pubDate = new Date(post.publishedAt).toLocaleDateString('zh-CN', { year: 'numeric', month: 'long', day: 'numeric', hour: '2-digit', minute: '2-digit' })
          ...
          <div className="flex items-center gap-2 flex-wrap">
            <div className="w-7 h-7 rounded-lg flex items-center justify-center" style={{ backgroundColor: `${platformColor}20` }}>
              <img src={`https://cdn.simpleicons.org/${post.platform === 'unknown' ? 'github' : post.platform}/${platformColor.replace('#','')}`}
                className="w-4 h-4 object-contain" onError={e => ((e.target as HTMLImageElement).style.display='none')} alt={post.platform} />
            </div>
            <span className="text-sm font-bold text-slate-700 dark:text-slate-200">{post.handle || post.platform}</span>
            <span className={`text-[10px] font-black px-2 py-0.5 rounded-full ${statusCls}`}>{STATUS_LABELS[post.status] ?? post.status}</span>
            <span className="text-[10px] font-black px-2 py-0.5 rounded-full text-white" style={{ backgroundColor: ctColor }}>{post.contentType}</span>
          </div>
          ...
          {post.mediaUrls?.length > 0 && (
            <div>
              ...
              <div className="flex gap-2 flex-wrap">
                {post.mediaUrls.slice(0, 4).map((u: string, i: number) => (
