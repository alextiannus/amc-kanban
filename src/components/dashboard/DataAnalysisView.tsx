...
export default function DataAnalysisView() {
      ...
      await new Promise<void>((resolve, reject) => {
        ...
        xhr.onload = () => {
          ...
          if (xhr.status >= 200 && xhr.status < 300) {
            ...
            reject(new Error(json.error || t('上传截图失败', 'Failed to upload screenshot')))
          }
        }

        xhr.onerror = () => {
          reject(new Error(t('网络连接错误，上传失败', 'Network connection error, upload failed')))
        }

        xhr.send(formData)
      })

      setNotification({ type: 'success', message: t('截图上传并更新成功！', 'Screenshot uploaded and updated successfully!') })
      loadData()
    } catch (err) {
      alert(err instanceof Error ? err.message : t('上传失败', 'Upload failed'))
    } finally {
  ...
  const handleInstagramLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!res.ok) {
        throw new Error(json.error || t('登录并抓取快照失败', 'Failed to log in and capture snapshot'))
      }
      setNotification({
        type: 'success',
        message: language === 'en'
          ? `Instagram account ${loginModalAccount.handle} logged in and screenshotted successfully!`
          : `Instagram 账号 ${loginModalAccount.handle} 登录并截屏成功！`
      })
      ...
      alert(err instanceof Error ? err.message : t('登录失败', 'Login failed'))
    } finally {
      setLoggingIn(false)
    }
  }
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      if (!res.ok) {
        const json = await res.json().catch(() => ({}))
        throw new Error(json.error || t('加载账号展现数据失败', 'Failed to load account snapshots data'))
      }
      const json = await res.json()
      setItems(json.results || [])
    } catch (err) {
      setError(err instanceof Error ? err.message : t('加载失败', 'Failed to load data'))
    } finally {
      setLoading(false)
    }
  }, [sortOrder])
  ...
  const triggerCrawler = async () => {
    ...
    try {
      ...
      if (res.ok) {
        setNotification({ type: 'success', message: t('已成功在后台启动截图抓取任务，请稍后刷新页面查看结果。', 'Successfully started screenshot capture task in the background. Please refresh the page later to check results.') })
        setTimeout(() => {
          loadData()
        }, 5000) // Wait 5 seconds and reload
      } else {
        const json = await res.json().catch(() => ({}))
        throw new Error(json.error || t('触发截图抓取失败', 'Failed to trigger screenshot capture'))
      }
    } catch (err) {
      setNotification({ type: 'error', message: err instanceof Error ? err.message : t('触发失败', 'Failed to trigger') })
    } finally {
    ...
    <div className="flex-1 p-6 md:p-8 space-y-6">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 border-b border-slate-150 dark:border-slate-800 pb-5">
        <div>
          <h2 className="text-2xl font-black text-slate-900 dark:text-white flex items-center gap-2.5">
            ...
            <span>{t('账号展现看板', 'Account Snapshots')} (Data Analysis)</span>
          </h2>
          <p className="text-xs text-slate-400 font-bold mt-1">{t('展示各品牌社交媒体账号的最新真实主页快照，由 AMC Researcher 定期回采更新。', 'Displays latest live homepage snapshots of each brand\'s social media accounts, regularly captured and updated by AMC Researcher.')}</p>
        </div>
        <div className="flex items-center gap-3">
          <button
            ...
            title={t('在大屏电视上播放最新截图', 'Play latest screenshots on large screen TV')}
          >
            <Tv className="w-4 h-4 text-indigo-400" />
            <span>{t('电子大屏展示', 'TV Display Board')}</span>
          </button>
          <button
            onClick={loadData}
            disabled={loading}
          ...
          >
            ...
            <span>{triggeringCrawler ? t('正在抓取...', 'Crawling...') : t('重新抓取全部快照', 'Recrawl All Snapshots')}</span>
          </button>
        </div>
      </div>
      ...
      <div className="bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 p-4 rounded-2xl shadow-sm flex flex-wrap items-center gap-4">
        ...
        <div className="relative flex-1 min-w-[200px]">
          ...
          <input
            ...
            placeholder={t('搜索账号名、品牌或主理人...', 'Search by account, brand or owner...')}
        ...
        <div className="flex items-center gap-2">
          <span className="text-xs font-bold text-slate-400 uppercase">{t('品牌', 'Brand')}</span>
          <select
            value={selectedBrand}
        ...
        {/* Filter Platform */}
        <div className="flex items-center gap-2">
          <span className="text-xs font-bold text-slate-400 uppercase">{t('平台', 'Platform')}</span>
          <select
        ...
        {/* Filter Owner */}
        <div className="flex items-center gap-2">
          <span className="text-xs font-bold text-slate-400 uppercase">{t('主理人', 'Owner')}</span>
          <select
            value={selectedOwner}
            onChange={(e) => setSelectedOwner(e.target.value)}
            className="bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-xl py-2 px-3 text-xs font-bold text-slate-700 dark:text-slate-300 outline-none focus:border-indigo-500"
          >
            <option value="all">{t('全部主理人', 'All Owners')}</option>
        ...
        >
          ...
          <span>{t('排序: 品牌名', 'Sort: Brand Name')} ({sortOrder === 'asc' ? 'A-Z' : 'Z-A'})</span>
        </button>
      </div>
      ...
      {loading ? (
        <div className="p-12 flex flex-col items-center justify-center min-h-[40vh] bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-3xl shadow-sm">
          ...
          <p className="text-sm font-semibold text-slate-400">{t('正在读取快照看板中...', 'Loading snapshot board...')}</p>
        </div>
        ...
        <div className="p-12 text-center bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-3xl shadow-sm min-h-[40vh] flex flex-col items-center justify-center">
          ...
          <h3 className="text-base font-extrabold text-slate-700 dark:text-slate-200">{t('未找到符合条件的账号快照', 'No matching account snapshots found')}</h3>
          <p className="text-xs text-slate-400 mt-1 max-w-sm">{t('请检查您的过滤器，或尝试点击“重新抓取全部快照”生成测试快照数据。', 'Please check your filters, or click "Recrawl All Snapshots" to capture snapshots.')}</p>
        </div>
      ) : (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-6">
          {filteredItems.map((item) => {
            ...
            const getPlatformLabelText = (platformId: string) => {
              const badge = PLATFORM_BADGES[platformId]
              if (!badge) return platformId.toUpperCase()
              if (platformId === 'xiaohongshu') {
                return t('小红书', 'RED / Xiaohongshu')
              }
              return badge.label
            }
            const platformLabel = getPlatformLabelText(item.platformId)
              ...
              >
                ...
                {isUploading && uploadingAccountId === item.accountId ? (
                  <div className="relative w-full aspect-[2/3] bg-slate-950/85 backdrop-blur-sm rounded-lg flex flex-col items-center justify-center text-white p-4 z-10 animate-in fade-in duration-200">
                    ...
                    <span className="text-[11px] font-black text-slate-200 mt-3 tracking-wide">{t('正在上传截图...', 'Uploading screenshot...')}</span>
                  ...
                  <div className="relative w-full aspect-[2/3] flex flex-col items-center justify-center p-4 text-center text-slate-450 bg-slate-50 dark:bg-slate-950/20 rounded-lg border border-dashed border-slate-200 dark:border-slate-800/60">
                    ...
                    <p className="text-[9px] text-slate-400 mt-2 max-w-[130px] leading-relaxed font-semibold">
                      {t('点击卡片登录并获取自动截图，或手动上传最新快照。', 'Click card to log in and auto-capture, or manually upload latest snapshot.')}
                    </p>
                  </div>
                )}
              </div>
              ...
              <div className="flex-1 bg-slate-950 p-6 flex items-center justify-center min-h-[300px] md:min-h-0 overflow-y-auto max-h-[50vh] md:max-h-none border-b md:border-b-0 md:border-r border-slate-150 dark:border-slate-800">
                {selectedAccount.latestSnapshot ? (
                  ...
                  <div className="flex flex-col items-center justify-center p-6 text-center text-slate-500">
                    ...
                    <p className="text-sm font-bold text-slate-400">{t('暂无该账号的抓取快照', 'No captured snapshot for this account')}</p>
                  </div>
                )}
              </div>
              ...
              <div className="w-full md:w-80 p-6 flex flex-col justify-between bg-slate-50/50 dark:bg-slate-900/50 shrink-0 space-y-6">
                <div className="space-y-5">
                  ...
                  <div className="space-y-3.5">
                    <h4 className="text-xs font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest">{t('账号明细', 'Account Details')}</h4>
                    <div className="space-y-2 bg-white dark:bg-slate-950 p-3 rounded-2xl border border-slate-200 dark:border-slate-855 shadow-sm text-xs">
                      <div className="flex justify-between py-1">
                        <span className="text-slate-450 font-bold">{t('平台渠道', 'Platform Channel')}</span>
                        <span className="font-black text-slate-800 dark:text-slate-200 uppercase">{selectedAccount.platformId}</span>
                      </div>
                      <div className="flex justify-between py-1">
                        <span className="text-slate-450 font-bold">{t('粉丝数量', 'Followers')}</span>
                      ...
                      {selectedAccount.ratingScore !== null && (
                        <div className="flex justify-between py-1">
                          <span className="text-slate-450 font-bold">{t('评分星级', 'Rating Score')}</span>
                          <span className="font-black text-slate-800 dark:text-slate-200">⭐ {selectedAccount.ratingScore.toFixed(1)}</span>
                        </div>
                      )}
                      <div className="flex justify-between py-1">
                        <span className="text-slate-450 font-bold">{t('更新时间', 'Last Updated')}</span>
                        <span className="font-black text-slate-800 dark:text-slate-200">
                          {selectedAccount.latestSnapshot 
                      ...
                      <div className="flex justify-between py-1">
                        <span className="text-slate-450 font-bold">{t('快照状态', 'Snapshot Status')}</span>
                        <span className="font-black">
                          {selectedAccount.latestSnapshot?.isUserUploaded ? (
                            <span className="text-emerald-500">{t('👤 用户手动上传', '👤 Manual Upload')}</span>
                          ) : selectedAccount.latestSnapshot?.isReal ? (
                            <span className="text-blue-500">{t('🤖 AI 真实抓取', '🤖 AI Crawled')}</span>
                          ) : selectedAccount.latestSnapshot ? (
                            <span className="text-amber-500">{t('⚠️ 未验证快照', '⚠️ Unverified')}</span>
                          ) : (
                            <span className="text-slate-400">{t('无数据', 'No Data')}</span>
                          )}
                        </span>
                      </div>
                    </div>
                  ...
                  <div className="space-y-2">
                    <h4 className="text-xs font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest">{t('账号主理人', 'Account Owner')}</h4>
                    <div className="flex flex-wrap gap-1.5">
                      {selectedAccount.owners.length === 0 ? (
                        <span className="text-xs text-slate-450 italic">{t('未绑定主理人', 'No Owner Bound')}</span>
                      ) : (
                ...
                <div className="space-y-2.5 pt-4 border-t border-slate-200 dark:border-slate-800">
                  {selectedAccount.profileUrl && (
                    ...
                    >
                      <Store className="w-4 h-4" /> {t('访问主页', 'Visit Homepage')}
                    </a>
                  )}
                  ...
                  >
                    <Upload className="w-4 h-4" /> {t('手动上传最新截图', 'Upload Screenshot')}
                  </button>
                  ...
                  >
                    <span>{t('🔑 重新登录并抓取', '🔑 Re-login & Crawl')}</span>
                  </button>
                </div>
              </div>
            </div>
          ...
          >
            <div className="flex justify-between items-center pb-2 border-b border-slate-100 dark:border-slate-800">
              <h3 className="text-sm font-black text-slate-900 dark:text-white">
                {t('Instagram 登录授权', 'Instagram Login Authorization')}
              </h3>
              <button 
                type="button"
                disabled={loggingIn}
            ...
            <div className="text-xs text-slate-500 dark:text-slate-400 leading-relaxed">
              {t('请输入账号', 'Please enter the Instagram credentials for')} <strong>{loginModalAccount.handle}</strong>{t('的 Instagram 登录凭证。系统将启动后台 Playwright 浏览器模拟登录并安全保存会话 Cookies，以支持自动截图抓取。', '. The system will start a background Playwright browser instance to log in and safely save session Cookies to support automated screenshot captures.')}
            </div>

            <div className="space-y-3">
              <div>
                <label className="block text-[10px] font-bold text-slate-400 uppercase mb-1">{t('手机号、用户名或邮箱', 'Phone, Username or Email')}</label>
                <input
                  type="text"
                  required
                  value={loginUsername}
              ...
              <div>
                <label className="block text-[10px] font-bold text-slate-400 uppercase mb-1">{t('密码', 'Password')}</label>
                <input
                  type="password"
                  required
                  value={loginPassword}
              ...
              >
                {loggingIn ? (
                  <>
                    ...
                    <span>{t('登录授权中...', 'Authorizing...')}</span>
                  </>
                ) : (
                  <span>{t('确认并登录', 'Confirm & Login')}</span>
                )}
              </button>
            </div>
          </form>
