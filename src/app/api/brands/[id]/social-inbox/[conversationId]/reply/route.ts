...
import { postfastReplyInboxItem } from '@/lib/integrations/postfast'
...
export async function POST(request: Request, { params }: Params) {
  ...
  if (!item.canReply || (item.replyWindowEndsAt && item.replyWindowEndsAt <= new Date())) return NextResponse.json({ error: 'PostFast does not allow a public reply for this item' }, { status: 409 })
  if (item.maxReplyLength && body.text.length > item.maxReplyLength) return NextResponse.json({ error: `Reply exceeds PostFast maximum length (${item.maxReplyLength})` }, { status: 400 })
  ...
  const scope = `postfast-inbox:reply:${id}:${body.itemId}`
  ...
  const result = await postfastReplyInboxItem({ apiKey: brand.postfastApiKey, itemId: body.itemId, text: body.text, idempotencyKey: key })
  ...
  const response = result.success ? { ok: true } : { error: result.error || 'PostFast reply failed' }
  await completeInboxWrite(scope, key, response, result.success ? 200 : 502)
  if (result.success) await writeAuditLog({
    actor: { id: session.user.id, type: 'HUMAN' }, action: 'POSTFAST_INBOX_PUBLIC_REPLY',
