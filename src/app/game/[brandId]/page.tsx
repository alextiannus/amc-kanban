...
export default function GameH5Page() {
  ...
  const fetchStatus = () => {
    ...
    fetch(`/api/game/status?brandId=${brandId}&sessionId=${sessionId}`)
      .then(res => res.json())
      .then(data => {
  ...
  const submitTask = async () => {
    ...
    if (config?.taskPhotoEnabled && !copyrightAgreed) return
      ...
      <main className="w-full max-w-md flex flex-col items-center gap-6 z-10 pb-16">
        ...
        {(config?.taskPhotoEnabled || config?.taskReviewEnabled) && (
          <div className="w-full rounded-2xl bg-white/5 border border-white/10 backdrop-blur-xl p-4 flex flex-col gap-4 shadow-xl">
            ...
            {/* Copyright agreed */}
            {config?.taskPhotoEnabled && (
              <label className="flex items-start gap-2 cursor-pointer mt-1">
                <input 
                  type="checkbox" 
                  checked={copyrightAgreed} 
                  onChange={(e) => setCopyrightAgreed(e.target.checked)}
                  className="mt-0.5 rounded border-slate-700 bg-slate-900 text-blue-500 focus:ring-blue-500/20"
                />
            ...
            <button
              disabled={
                ...
                (config?.taskPhotoEnabled && !copyrightAgreed) ||
                isSubmittingTask
              }
              onClick={submitTask}
