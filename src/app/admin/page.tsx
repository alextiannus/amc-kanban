      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="col-span-1">
          <div className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700">
            <h2 className="text-lg font-semibold mb-4 dark:text-gray-200">Add New Entity</h2>
            <form onSubmit={handleAddUser} className="space-y-4">
              ...
              <div>
                <label className="block text-sm font-medium mb-1 dark:text-gray-300">Email Address (ID)</label>
                <input 
                  type="email" 
                  required 
                  value={email}
        ...
        <div className="md:col-span-2 space-y-6">
          <div className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700">
            <h2 className="text-lg font-semibold mb-4 dark:text-gray-200">Human Users</h2>
            <ul className="divide-y divide-gray-200 dark:divide-gray-700">
              {humans.map(h => (
                <li key={h.id} className="py-3 flex justify-between items-center">
                  <div>
                    <p className="font-medium dark:text-white">{h.email}</p>
                    <p className="text-xs text-gray-500">Permitted Agents: {h.permittedAgents.length}</p>
                  </div>
              ...
              {humans.length === 0 && <p className="text-gray-500 text-sm">No human users found.</p>}
            </ul>
          </div>

          <div className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700">
            <h2 className="text-lg font-semibold mb-4 dark:text-gray-200">AI Agents</h2>
            <ul className="divide-y divide-gray-200 dark:divide-gray-700">
              {agents.map(a => (
                <li key={a.id} className="py-3">
                  <p className="font-medium dark:text-white">{a.email}</p>
                  <p className="text-xs text-gray-500">ID: {a.id}</p>
                </li>
              ))}
              {agents.length === 0 && <p className="text-gray-500 text-sm">No AI agents found.</p>}
            </ul>
          </div>
        </div>
      </div>
      ...
      {selectedHuman && (
        <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50">
          <div className="bg-white dark:bg-gray-800 w-full max-w-md rounded-xl shadow-xl p-6">
            <h2 className="text-lg font-bold mb-4 dark:text-white">Manage Agents for {selectedHuman.email}</h2>
            <div className="space-y-3 max-h-64 overflow-y-auto mb-6">
              {agents.map(agent => (
                <label key={agent.id} className="flex items-center gap-3 p-2 hover:bg-gray-50 dark:hover:bg-gray-700 rounded cursor-pointer">
                  ...
                  />
                  <span className="text-sm font-medium dark:text-gray-200">{agent.email}</span>
                </label>
              ))}
              {agents.length === 0 && <p className="text-sm text-gray-500">No agents available to assign.</p>}
            </div>
