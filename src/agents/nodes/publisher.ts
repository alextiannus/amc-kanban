...
export async function publisherNode(state: any) {
  ...
  if (state.aiFailed) {
    console.log(`AI Copywriting failed for task ${taskId}. Deleting draft ${existingDraftId} and failing task.`);
    if (existingDraftId) {
      try {
        ...
        console.log(`Deleted failed draft ${existingDraftId} and its ActionItems.`);
      } catch (e) {
        console.warn(`Failed to delete draft ${existingDraftId}:`, e);
      }
    }
