...
/**
 * Call Gemini with multi-turn history and optional function calling support.
 *
...
export async function callGeminiChat(
  ...
  if (!apiKey) {
    console.warn('[GeminiChat] API key missing')
  ...
  try {
    ...
    if (!candidate) return { reply: '不好意思，您能再说一遍吗？', action: 'NONE' }
    ...
    if (!text) return { reply: '不好意思，您能再说一遍吗？', action: 'NONE' }
    ...
    console.error('[GeminiChat] Request failed:', error)
    return { reply: '不好意思，您能再说一遍吗？', action: 'NONE' }
  }
}
