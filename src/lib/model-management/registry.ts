...
export async function createConnection(input:{name:string;protocol:string;baseUrl:string;secret:string;previousId?:string},actorId:string){
  ...
  const id=randomUUID()
  await db.$executeRawUnsafe('INSERT INTO "ModelConnection" (id,"previousId",name,protocol,"baseUrl","encryptedSecret","secretFingerprint","actorId") VALUES ($1,$2,$3,$4,$5,$6,$7,$8)',id,input.previousId||null,input.name.trim(),input.protocol,input.baseUrl.replace(/\/+$/,''),encryptSecret(input.secret.trim()),secretFingerprint(input.secret.trim()),actorId)
  return {id}
}
export async function createModel(input:{connectionId:string;legacyId?:string;definition:ModelDefinition},actorId:string){
  const raw=input.definition
  const d:ModelDefinition=raw&&{name:raw.name,modelName:raw.modelName,capabilities:raw.capabilities,inputCapabilities:raw.inputCapabilities,timeoutMs:raw.timeoutMs,maxRetries:raw.maxRetries,temperature:raw.temperature,jsonMode:raw.jsonMode,maxTokensByTask:raw.maxTokensByTask,videoConstraints:raw.videoConstraints,costMetadata:raw.costMetadata}
  ...
  const id=randomUUID()
  await db.$executeRawUnsafe('INSERT INTO "ModelCatalogEntry" (id,"connectionId","legacyId",definition,"actorId") VALUES ($1,$2,$3,$4::jsonb,$5)',id,input.connectionId,input.legacyId||null,JSON.stringify(d),actorId)
  return {id}
}
export async function overview(){
  const [connections,models,revisions,calls,validations]=await Promise.all([
  ...
  return {connections,models,revisions,calls,validations,current:await runtimeConfig(),capabilities:CAPABILITIES}
}
export async function publish(selection:Selection,expectedVersion:number|null,validationId:string,actorId:string){
