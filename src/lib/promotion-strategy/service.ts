...
function buildPromotionPoints(input: {
  ...
  const limit = input.servicePlanId === 'starter' ? 4 : input.servicePlanId === 'advanced' ? 8 : 6
  ...
  return fallback.map((sellingPoint, index) => ({
    ...
    requestedCandidateCount: input.servicePlanId === 'starter' ? 3 : 5,
...
function normalizeServicePlan(value: unknown) {
  ...
  return ['starter', 'essential', 'advanced'].includes(plan) ? plan : 'essential'
}
