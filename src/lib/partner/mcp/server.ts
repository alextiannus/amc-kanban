...
async function ensureCalendarAccount(brandId: string, platform: string) {
  ...
  const account = await prisma.socialAccount.findFirst({
    where: { unboundAt: null, brandId, platformId },
...
export function createAmcMcpServer(auth: AuthPrincipal | string, credentialToken?: string) {
  ...
  const publishContentHandler = async ({ brandId, idempotencyKey, platform, instagramPublishType, caption, mediaStorageKeys, mediaUrls, hashtags, scheduledAt, accountId, gbpLocationId, postfastControls }: { brandId: string; idempotencyKey: string; platform: 'instagram' | 'tiktok' | 'xiaohongshu' | 'facebook' | 'youtube' | 'x' | 'linkedin' | 'threads' | 'bluesky' | 'pinterest' | 'snapchat' | 'telegram' | 'google'; instagramPublishType?: 'TIMELINE' | 'REEL' | 'STORY'; caption: string; mediaStorageKeys?: string[]; mediaUrls?: string[]; hashtags?: string[]; scheduledAt?: string; accountId?: string; gbpLocationId?: string; postfastControls?: Record<string, unknown> }) => {
    ...
    const account = await prisma.socialAccount.findFirst({
      where: { unboundAt: null, id: accountId.trim(), brandId },
    ...
    const result = await postfastPublish({ apiKey: brand.postfastApiKey, brandId, platform, instagramPublishType, caption, mediaStorageKeys, mediaUrls, hashtags, scheduledAt, accountId: account.postfastAccountId, gbpLocationId, ...postfastControls })
      ...
      if (resolvedAccountId) {
        if (resolvedAccountId.startsWith('unconfigured_')) {
          ...
          const localAccount = await prisma.socialAccount.findFirst({
            where: { unboundAt: null, id: resolvedAccountId, brandId },
          ...
          if (!localAccount) {
            ...
            const account = await prisma.socialAccount.findFirst({ where: { unboundAt: null, brandId, postfastAccountId: externalAccount.id } })
