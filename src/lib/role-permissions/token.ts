...
export function signAccessIdentity(principal: AuthPrincipal, brandId?: string) {
  ...
  const payload = Buffer.from(JSON.stringify({ sub: principal.userId, email: principal.email, role: principal.globalRoles.includes('ADMIN') ? 'ADMIN' : principal.globalRoles[0], authVersion: principal.authVersion, brandId, exp: Math.floor(Date.now() / 1000) + 86400 })).toString('base64url')
