...
assert.match(internalApi, /requestedMediaKind === 'image' \|\| requestedMediaKind === 'video'/)
assert.match(internalApi, /mimeType: \{ startsWith: 'image\/' \}/)
assert.match(internalApi, /mimeType: \{ startsWith: 'video\/' \}/)
assert.match(internalApi, /orderBy: \{ createdAt: 'desc' \}/)
for (const field of ['total', 'page', 'pageSize', 'totalPages']) assert.match(internalApi, new RegExp(`\\b${field}\\b`))
