...
function testLegacyEntrypointsUseFacade() {
  ...
  assertIncludes(bulkRoute, "import { generateMultiPlatformWithRemoteContentService } from '@/lib/amc-content/remoteContentService'", 'bulk route multi-platform content import')
  assertIncludes(bulkRoute, 'const multiResult = await generateMultiPlatformWithRemoteContentService({', 'bulk route multi-platform content call')
  ...
  assertIncludes(dashboardAssets, '/drafts/batch-trigger-copywriter', 'asset AI batch create uses batch content generation')
  assertNotIncludes(dashboardAssets, '/trigger-copywriter', 'asset AI batch create must not fan out legacy copywriter graph requests')
...
function testViralCopyScriptBridge() {
  ...
  assertIncludes(batchTriggerRoute, 'generateMultiPlatformWithRemoteContentService', 'batch trigger uses amc-content multi-platform generation')
  assertIncludes(batchTriggerRoute, '[batch-trigger-copywriter]', 'batch trigger logs production route hits')
  ...
  assertIncludes(postEditDrawer, "throw new Error(batchJson.error || 'AI 创作失败')", 'copywriter modal surfaces batch content generation failures')
  ...
  assertIncludes(dashboardAssets, "throw new Error(batchData.error || 'amc-content 文案创作失败')", 'asset new-publish surfaces direct content failures')
  assertIncludes(draftManagement, '/drafts/batch-trigger-copywriter', 'draft management multi-draft regeneration uses batch content generation')
  assertIncludes(dashboardCalendar, '/drafts/batch-trigger-copywriter', 'calendar multi-draft regeneration uses batch content generation')
  assertNotIncludes(draftManagement, 'createdDrafts.map(d => triggerCopywriter(d.id, true))', 'draft management must not fan out legacy copywriter graph requests')
  assertNotIncludes(dashboardCalendar, 'createdDrafts.map(d => triggerCopywriter(d.id, true))', 'calendar must not fan out legacy copywriter graph requests')
