...
assert(serviceSource.includes('creativeMatchStatus?: CalendarCreativeMatchStatus'))
assert(serviceSource.includes('resolveCalendarCreativeCandidateWithRetry'))
assert(serviceSource.includes('creativeMatchStatus: match.status'))
assert(serviceSource.includes('creativeMatchStatus,'))
assert(profileSource.includes("item.creativeMatchStatus === 'no_candidate_after_retry'"))
assert(profileSource.includes('未找到灵感 · 平台规则原创'))
