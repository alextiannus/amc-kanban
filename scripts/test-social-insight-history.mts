...
assert(route.includes("status: { in: ['published', 'done'] }"))
assert(route.includes("id: d.platformPostId ? `internal_${d.platformPostId}` : `internal_draft_${d.id}`"))
...
assert(backfill.includes("process.argv.includes('--internal-only')"))
assert(backfill.includes("new Date('2026-01-01T00:00:00.000Z')"))
assert(backfill.includes("externalId: draft.platformPostId || draft.id"))
assert(route.includes('kpis: { totalPosts, totalEngagement, totalImpressions, avgReach, totalLikes, avgEngRate }'))
assert(backfill.includes('raw: { draftId: draft.id }'))
assert(backfill.includes("internalOnly, from: validFromDate.toISOString()"))
