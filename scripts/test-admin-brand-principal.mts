...
let state:any
function reset(){state={brand:{id:'b',ownerId:'owner',name:'Before'},members:[{userId:'owner',role:'OWNER',active:true},{userId:'lead',role:'PRINCIPAL',active:true},{userId:'editor',role:'EDITOR',active:true}],audit:[]}}
reset()
const tx:any={
 brand:{update:async({data}:any)=>Object.assign(state.brand,data),findUnique:async()=>state.brand},
 brandAgent:{updateMany:async()=>{},upsert:async()=>{}},
 marketingCrew:{findUnique:async()=>({id:'crew'})},
 crewMember:{findMany:async()=>state.members.filter((m:any)=>m.active),updateMany:async({where}:any)=>{for(const m of state.members)if(!where.userId.notIn.includes(m.userId))m.active=false}},
 auditLog:{create:async({data}:any)=>state.audit.push(data)},
}
const db={...tx,brandSubscription:{findFirst:async()=>null},user:{count:async({where}:any)=>where.id.in.length},$transaction:async(fn:any,opts:any)=>{assert.equal(opts.isolationLevel,'Serializable');const before=structuredClone(state);try{return await fn(tx)}catch(e){state=before;throw e}}}
const general=compile('src/app/api/admin/brands/[id]/route.ts',{
 ...
 '@/lib/brand-operations/service':{OperationsError}, '@/lib/brand-operations/http':http,
})
const save=(agentIds:string[])=>general.PATCH(new Request('http://localhost/api/admin/brands/b',{method:'PATCH',body:JSON.stringify({name:'After',agentIds})}),context)
