...
- **电子屏专属大屏播放页 (`/presentation/share-tv-display-board`)**：
  - **极致暗色浸润背景**：页面采用 `bg-slate-950` 暗色浸润背景，并将当前播放快照的高斯模糊（blur-3xl）与低不透明度缩放图作为自适应背景，营造高端的大气影院氛围（Ambient Glow）。
  - **动态播放特效 (Ken Burns)**：图片支持多种播放过渡效果（包括渐变 Fade、幻灯 Slide、3D翻转 Flip，以及默认的 Ken Burns 柔和缩放移动特效），极大增强静态截图的动感与品质感。
  - **数据滚动自动循环**：支持设定定时器自动过渡下一张，界面顶部配有细长渐变进度条实时提示，让大屏自运转清晰明了。
  - **多维度视图模式**：提供单图轮播与网格看板陈列两种视图模式的快捷切换。
  - **时间与日期挂件**：右上角配备醒目的数字电子时钟与本地化日期，可兼做办公区电子挂钟。
  - **扫码控制与分享二维码**：右下角通过 `qrcode` 库动态生成当前 URL 的分享/控制二维码，电子屏前的团队人员可随时扫码共享或在手机端同步查看。
  - **键盘快捷键支持**：实现 Space (播放/暂停)、ArrowLeft/Right (切图)、F (全屏)、Q (二维码开关)、G (视图切换)、H (快捷键帮助) 完整的键盘操纵。
