...
- 品牌运营行可点击导航，品牌名为可键盘访问的真实链接：管理员打开 `/admin?tab=brands&brandId=<id>` 并直接展开对应品牌编辑；主理人打开 `/board?tab=dashboard&brandId=<id>`，沿用现有品牌页面权限。不提升权限；不改写数据，选中文字不触发跳转。此导航已于 2026-09-21 随版本 `776ecf09` 部署。
