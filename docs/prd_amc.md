...
- 页面语言直接跟随顾客浏览器，不显示单独的文案语言或体验标签选择。首次进入时，AI 根据商家公开资料自动生成小红书和 Instagram 的中性品牌介绍草稿；不得冒充顾客到店、消费或推荐。
- Google Review 输入框默认隐藏；中文浏览器自动使用“这个商家不错”，其他语言浏览器使用 `This business is good.`，并结合品牌名称、品牌介绍、门店地点和公开菜单信息自动扩写，无需点击生成按钮。生成后的草稿仍可编辑并持续提示顾客确认内容准确；默认句同时可用于站内反馈，店员 PIN 确认后获得 5 积分。同一品牌和浏览器 session 每个商家营业日只奖励一次，公开发布不是积分或奖品条件。
- AI 分享草稿无需 PIN 即可预览、编辑和“复制并打开平台”，顾客也可完全不公开分享。页面不得使用“AI 好评”或“发布后兑奖”等引导性表述。
