...
*   **AI 自动化处理：** 
    *   **图片分析与推荐归类（代码已实现，待部署）：** 新上传图片自动使用 `doubao-seed-2.1-turbo` 分析，历史素材支持选中分析及全库未分析补全。优先采用品牌行业资料，逐图保存内容类型、说明及标签，汇总为推荐文件夹；用户可改名、选择已有目录、调整图片或跳过，确认后才移动素材。分析与实际目录独立，保留人工标签及发布状态。详细流程、接口和验收见 [素材库图片分析](asset-image-analysis.md)。
