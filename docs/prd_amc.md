...
*   **媒体准入与发布校验：**
    ...
    *   发布前根据目标平台再次校验数量、媒体组合、文件格式、尺寸/比例、编码、帧率和时长。Instagram 单视频默认按 Reel 发布；Instagram 与 TikTok 均不允许在同一草稿中混合图片和视频。
    *   不符合要求的素材直接拒绝并返回可定位到文件与字段的结构化错误（HTTP 422）；系统不自动裁剪、压缩或转码，也不把发布前校验失败的草稿改成 `failed`。
