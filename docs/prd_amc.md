...
- Kanban 的最新报告“查看”对 `ADMIN`、`AMC_PRINCIPAL` 使用 Growth SSO 深链 `/dashboard/reports/versions/:id?view=standalone`。已保存报告存在合法 Growth 版本 ID 时，这两个角色直接访问历史 Kanban `/dashboard/brands/:id/research-report` 地址也必须自动进入同一 SSO 沉浸页；该模式继续校验 Growth Session，但不渲染后台左侧导航、账户退出、返回报告库、分享和删除，只显示封面、目录、正文以及 PDF/Markdown 下载。普通 Growth 后台报告链接保持不变；品牌用户、API Key 请求或缺少 Growth 版本 ID 的历史报告仍使用 Kanban 本地报告页，不得借沉浸展示模式绕过授权或形成重定向循环。
