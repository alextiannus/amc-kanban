...
- **电子屏专属大屏播放页 (`/presentation/share-tv-display-board`)**：
  - **默认密集网格（100% 屏效）**：系统默认以高密度的网格形式展示所有账号的完整截图快照。无页眉、无任何元数据文字覆盖、无多余的背景色，实现全屏幕 100% 空间纯图化沉浸展示。
  - **异步错位变焦呼吸动效**：每个卡片的截图都独立且随机化绑定了 Ken Burns 舒缓变焦与微摇摆呼吸特效，其动画周期和延迟由随机数打散分布（25s~40s），从而呈现生动的错落流动墙效果，让静态画面呈现出动态生机。
  - **自动往复慢速平移滚动**：当快照数量超出当前屏幕视口高度时，页面会自动以极其平缓的慢速（requestAnimationFrame）进行上下往复纵向滚动，完全零干预循环播放。
  - **隐形底部多功能控制台**：控制面板默认完全透明隐藏（`opacity-0`），且不干扰内容交互；只有当鼠标滑入屏幕最下方 120px 的触碰感应区时，控制面板才会以精致的动效呈现（`opacity-100`），提供网格与单图轮播的自由切换，并可配置自动滚动状态、网格列数（4/6/8列）、滚动速度及呼吸动画开关等。
  - **键盘快捷键操纵**：支持 Space (暂停滚动/播放)、ArrowLeft/Right (切图)、F (全屏)、Q (二维码开关)、G (网格与单图切换)、H (快捷键说明) 的纯键盘操作。
