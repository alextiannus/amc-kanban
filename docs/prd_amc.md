...
视频工作区品牌列表和业务操作统一使用 Kanban 每次请求实时解析的品牌范围。签名身份中的 brandId 用于初始选择，不作为用户全部可操作品牌的唯一名单；选择其他已授权品牌时，草稿创建、查询、素材、声音、生成、归档与回收均按目标资源所属品牌校验。列表未指定品牌时非管理员只能使用已验证的默认品牌，不能查询全部项目。撤权立即生效，未授权品牌仍拒绝；后台任务及无实时权限上下文的内部调用保留原有单品牌校验，不能仅凭客户端 brandId 或角色放行。验收需覆盖默认品牌 A、授权品牌 B、未授权品牌 C、同令牌撤权、缺少品牌范围及项目 ID 越权。
