...
- 实际订阅指数据库真实关联记录，保留待激活、失败、取消、到期记录，排除无记录品牌。优先选择当前生效 ACTIVE 合约，否则选择最近创建的订阅；选定订阅 feeWaived=true 的品牌不进入列表、汇总、主理人筛选或发布次数统计，不得回退历史收费合约使其重新入选。历史免收记录不排除当前收费品牌，免收续约草稿也不覆盖仍生效收费合约。免收排除规则已在本地实现，待生产发布。ACTIVE 根据开始/结束日期区分未开始与已到期。支持搜索、状态/主理人筛选、到期/本月发布排序及分页。
