...
*   **Postiz-Style 界面重构设计（Stitch 统一源）：**
    ...
    *   **未配置平台内容创作与草稿保存**：支持对未曾配置/连接好的社交媒体平台进行内容创作（包括人类手动输入与 AI Copywriter 自动生成），能够以正确的平台名称将草稿记录保存到数据库中，并在日历和草稿列表中清晰标识，以便品牌主复制内容进行手动跨平台发布。
...
*   **输出内容**：针对已配置及未配置的所有目标平台（如小红书、Instagram、Facebook）并行生成个性化内容与标签（Hashtags）。

### 3. 首页卡片左右滑动预览 (Inline Horizontal Carousel Preview)
*   **界面布局**：生成的结果不在弹窗或全屏遮罩中展示，而是直接在 `amc-mm` 首页渲染为内嵌的卡片横滑区域。
*   **横滑交互**：使用 CSS Snap Scroll (`snap-x snap-mandatory overflow-x-auto`)，支持老板左右手势滑动浏览不同平台的草稿卡片，每张卡片支持文案和标签的二次编辑。

### 4. 智能排期与保存草稿 (Smart Schedule & Save Draft)
*   **业务逻辑**：用户点击“智能排期”后，选中的平台内容将被提交至后台 Post 管理数据库中，保存为 `draft`（草稿）状态。
...
*   **分流逻辑**：用户点击“直接发布”时：
    ...
    *   **未连接的平台（DisplayName 标明未配置）**：内容卡片保留在首页展示中，不自动提交。界面出现 high 亮提示，提供“复制手动发布文案”按钮，支持用户一键复制正文+标签前往相应应用进行手动发布。
