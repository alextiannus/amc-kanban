...
### 账号快照展示与布局优化
- **快照卡片无遮挡展示**：移除了覆盖在卡片图片上层的所有悬浮遮挡元素（包括：左上角验证状态标签、右上角平台类型标签、底部账号名/品牌名半透明背景及文字、鼠标悬浮遮罩等），确保截图本身 100% 干净无遮挡。
- **元数据信息下移**：将账号名、品牌名、平台标志、快照类型（用户上传/AI 真实抓取）以及更新时间全部下移至截图卡片下方的独立区块展示，以满足“陈列在一起做比较”的上下文识别与对比需求。
