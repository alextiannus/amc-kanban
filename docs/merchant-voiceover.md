...
The first ready voice becomes default; renaming preserves default. Dedicated brand voice endpoints own all profile writes; brand-scoped transactions serialize updates. MM offers the brand default, another ready voice, a controlled system voice or no narration per video. Jobs snapshot voice/configuration and narration. Kanban persists each generated segment; MM passes audio URLs into Content assembly. Retries reuse successful segments and shots. Complete narration is preserved with at most two seconds of final-frame extension per shot, shifted subtitles and reduced music. Video providers and assistant voices remain unchanged.
...
- MM video jobs store their voice snapshot and progress in `VideoProductionJob.voiceoverState`. No voice parameters preserves the existing automatic narration decision. New MM requests explicitly select on/off. Ready unbound legacy voices need re-recording; they cannot silently switch accounts.
