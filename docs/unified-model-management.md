...
4. In the single model management panel, click Initialize existing configuration and review the persisted, redacted report. Disabled models enter the catalog without becoming selected. Content must be deployed with the complete catalog export before initialization can finish. Resolve active-route conflicts, missing credentials and capability warnings before activation. Import does not activate. Basic video has its own content:basic_video_generation exception. Existing asset analysis retains its CN Gateway connection.
