...
### 基础视频确认归档与回收站（本地已实现，待部署及线上存储联调）

基础视频合成成功后为“待确认”，人工点击“确认完成”才将最新有效完整成片写入所属品牌素材库的系统目录“AI视频”（英文 AI Videos）。系统目录对新老品牌始终可见且不可删除或重命名。成功入库的项目移入只读“已完成”列表，保留脚本、分段及来源，提供前往素材库入口；入库失败保留草稿，可幂等重试。历史项目不自动入库。

基础视频项目删除改为移入“视频回收站”，从删除时起保留 15×24 小时，期间可恢复至原列表，不提供人工立即清空。存在待执行或运行中任务时禁止确认和删除；已确认及已删除项目禁止修改和提交任务。启动时及每小时清理到期项目，清理任务持久化、失败重试、多实例互斥。只清理项目独有数据和文件，绝不删除素材库独立保存的成片或商家原始素材。所有入口沿用品牌权限并保留审计。
