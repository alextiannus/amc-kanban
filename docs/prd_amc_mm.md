...
### PostFast 互动与发布控制（当前规则）

- 发布与排期账号由 Kanban 按当前品牌解析，排除已归档品牌；后台大视频任务和重试固定使用任务的品牌 ID。共用 PostFast 配置的归档旧品牌不得阻断当前品牌发布，真正的多品牌歧义仍返回错误。（本地修复，待部署。）

- TikTok 草稿预览提供中英文「AI 生成的内容」开关，独立于文案只允许补空的限制；默认关闭，用户手动选择。修改独立保存并回显，保存中或失败时禁止发布，失败可重试。发布中、已提交平台排期及已发布时仅展示保存值；历史草稿缺少字段按关闭显示。使用现有草稿编辑授权，保存 `postfastControls.tiktokIsAigc` 且保留其他控制项。

- 品牌主可在移动端看到账号重新连接提醒、未读评论和需确认的互动建议；评论回复、Instagram 私信回复、隐藏和删除均明确展示目标账号、原评论与实际发送文本。
- AI 可准备回复建议，但不会自动发送私信、隐藏或删除评论。品牌主确认后，Kanban 使用 PostFast 返回的可回复状态、字数上限和私信窗口执行操作。
- 新建或编辑草稿可选择 Instagram 地理位置、首条评论、AI 内容标签和高级 Reel 设置；默认界面保持简洁，高级选项折叠呈现。TikTok 图文轮播可选择已授权音乐；GBP 活动或优惠帖可配置活动时间与 CTA。
- 数据页展示可用的 Instagram 保存率、Reel 跳过率和视频观看时长；没有平台数据时显示未提供，而不是显示零。
...
发布日历支持手工补录真实已发布帖文。平台账号下拉框的可见选择必须与提交到 Kanban 的 `accountId` 一致；账号异步加载、品牌切换或弹窗重开后，若原选择仍有效则保留，否则切换到当前可见的第一项。提交前再次校验账号有效性，禁止以界面显示 TikTok、内部却提交 Instagram 占位账号的失配状态保存。

发布日历月份完整加载（本次修复，待部署）：MM 调用 `GET /api/brands/:id/drafts?calendarStart=<ISO>&calendarEnd=<ISO>&page=1`，边界取浏览器本地月份起止时刻，结束时刻不包含。Kanban 按 `publishedAt → scheduledAt → createdAt → updatedAt` 的首个非空日期筛选后分页，每页 100 条，返回 `pagination`（`page`、`pageSize`、`total`、`hasMore`）。MM 加载全部页后统一显示日期标记、列表及状态数量。普通不带日期范围的草稿请求仍返回最近更新 100 条。切换品牌/月、刷新及编辑/删除/补录后重新加载；旧请求取消，失败明确提示并可重试，不显示空月结论。验收包含全品牌超过 100 条的历史月份、单月超过 100 条、月边界、快速切换、保存后更新及权限隔离。

已发布帖文预览支持修正链接与发布时间（本次开发，待部署）。点击链接或日期旁的编辑图标进入编辑，保存/取消统一处理这两个字段；链接本身保持单击在新标签页打开，不显示“Real post link saved.”。`PATCH /api/brands/:id/drafts/:draftId` 在既有品牌权限校验后允许已发布记录更新 `publishedAt`，仅接受有效且不晚于当前时间的日期；未传入时保留原值。修改只影响 AMC 中的记录，不触发第三方重新发布或改变 `scheduledAt`；保存后回读并刷新发布日历，日期归属以 `publishedAt` 为准。验收覆盖仅改链接、仅改日期、一起修改、取消、保存失败保留输入，以及未来/无效日期拒绝。

素材上传采用前后端双重校验：AMC-MM 在申请直传地址之前先检查文件类型、大小及可读取的图片/视频参数，服务端在确认上传时重新读取 OBS 原文件并以服务端结果为准。图片仅接受 JPEG/PNG/GIF/WebP 且不超过 10 MB，视频仅接受 MP4/MOV/WebM 且不超过 250 MB；不合规文件返回结构化错误，不自动裁剪、压缩或转码。发布时由 AMC 后端再按目标平台检查 Instagram/TikTok 参数，平台建议的图片/视频格式、大小、尺寸、比例、编码、帧率和时长仅返回 warning 并继续提交；无素材、媒体混合、数量非法、文件损坏或源文件不可访问仍阻止发布。历史图片超过 10 MB 时允许发布前读取并提示。Instagram 草稿可选 `TIMELINE`、`REEL` 或 `STORY`；未选择时单视频默认作为 `REEL`，其他合法图片组合默认作为 `TIMELINE`，且服务端在发布前校验发布类型与媒体组合兼容。
...
- Content 视频生成接入（本地已实现，待发布）：MM 已录入的声音由 Kanban 按品牌提供给 Content 人声音色选择器，新视频优先默认声音；录入和凭据管理继续保留在现有入口。试听及视频配音使用原配置，声音停用后阻止新的配音调用。Content 不要求重复录音或填写 Key。
...
## 自由视频生成（普通模式接入实施中，待部署验收）

无模板请求固定 `creationMode=text`、`scriptMode=manual`、`generationMode=per_shot`、`previewOnly=false`，原始描述和完整参考素材直接交给 Content，单条视频使用模型原生音频，不调用 AI 编剧或独立 TTS。增加时长选择，默认 10 秒、9:16、720p，前后端按 Content 当前模型能力校验；描述中的时长不覆盖选择值。标准模板保留专业模式。

MM 后端通过登录会话调用 Kanban `POST /api/content/access-identity`（brandId 必填），取得品牌范围的签名身份，以 `x-amc-access-identity` 调用 Content。身份不下发浏览器，服务 Token 不代替用户授权，不默认扩大商家权限。Content 负责创建、素材准备、估价确认、生成及合成；MM 持久化项目 ID 与制作模式，刷新恢复查询，普通模式重新生成走 Content 草稿/镜头接口，已接受、失败或结果未知的任务不自动重提。模板读取同步使用身份鉴权。401、403、503 与模型错误分别呈现，不硬编码错误供应商。

上线先部署 Kanban 身份接口，再部署 MM。验收覆盖授权商家、跨品牌、撤权、10 秒参考图与轻快音乐、纯文字和混合素材、超限时长、刷新恢复、模板及历史任务，分别记录本地验证、部署与真实成片证据。
...
## ImmediToday 订阅同步（本次开发，待部署验证）

MM 创建的订阅由 Kanban 持久化扫描器统一同步为 ImmediToday Sales Order；不依赖用户返回支付确认页。真实客户资料或品牌未就绪时记录待同步，失败自动重试。同一订阅只保留一个 ERP 订单引用。MM 不独立创建第二份订单、不把订单创建视为收款确认，也不绕过 ERP 的正式提交审核。
