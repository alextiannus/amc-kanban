...
管理员接口：GET /api/admin/role-permissions、PUT /api/admin/role-permissions/[role]；原权限总览继续作为有效授权诊断。Content 通过内部认证的 POST /api/internal/access/resolve 查询当前策略；不可用时受保护操作返回 503，不沿用旧授权。签名入口只证明身份，不能代替实时授权。旧凭证重新从 Kanban 进入。
