...
> 日期：2026-07-04
> 适用系统：amc-kanban、amc-content（权限总览）、amc-mm、外部 REST API、AMC MCP
> 文档性质：产品需求、技术方案与执行基线
> 优先级：P0
...
### 权限总览与账号诊断（本地已实现，未上线验收）

用户与权限管理中心增加只读“权限总览”，支持五角色矩阵、菜单搜索、仅看可用/冲突，以及从角色详情和账号列表进入。覆盖 Kanban 主导航、后台、个人菜单和 Content 主导航、Content Lab 子页；Growth 仅记录跳转入口。

菜单展示、页面入口、接口操作、品牌范围分别输出，状态为允许、禁止、需满足条件、待开放、规则冲突、未核实；不适用操作不伪造授权结果。复用实际菜单与授权函数，多角色不能简单拼接菜单。具体业务对象状态、订阅及第三方就绪条件未验证时必须明确提示，不将授权预测描述成成功操作。

账号诊断每次重新读取账号状态、显式角色、现有认证兼容角色、直接 Crew 与组织继承关系，选择品牌后重新计算；停用账号不能使用功能。当前认证兼容路径仍会读取 owner 关联，而 `/api/auth/me` 菜单角色只取账号本身，诊断需分别显示并标出差异，不在本期修改该行为。

接口：管理员网页会话专用 `GET /api/admin/access-overview` 和 `GET /api/admin/users/[id]/access-overview?brandId=...`，不支持 API Key 委托；响应 no-store，不包含密钥或凭证。Content 提供 `GET /v1/internal/access-overview`，使用现有内部服务令牌认证，返回版本化权限描述。版本不匹配或服务不可达时 Content 标为未核实，Kanban 结果仍可用。

已确认源码冲突：单独研究员可见 AI 角色库但 Kanban 入口拒绝；非管理员的部分工作日志权限与菜单展示不同。主理人视频生产需已授权品牌，Content 登录辅助函数仅允许管理员，但当前未挂载直接登录 HTTP 入口，标为未核实；当前使用 Kanban 签名入口。冲突只展示、不修复、不增权，不新增动态 RBAC 表。

验收覆盖五角色及组合、无角色、停用账号、直接/组织/撤销品牌授权、未选品牌、Content 异常与版本不匹配、非管理员访问拒绝和只读性。上线记录两个服务版本及代表账号实测；本地代码计算不能替代线上验证。
