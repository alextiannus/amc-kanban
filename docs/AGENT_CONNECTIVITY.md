...
*   **20:00 (评论监控与回复)**：
    1. 调用 `get_brand_reviews(brandId, status='unread')` 抓取未回复 Google/Yelp/社媒评论。
    2. 生成符合品牌调性的回复文案（好评致谢，中评改进，≤2星差评生成 `require_input` 任务挂起待主理人决策）。
    3. 调用 `execute_brand_action(actionType='reply_review')` 执行回复。
*   **23:00 (运营日志记忆整理)**：
